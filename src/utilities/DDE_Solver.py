...
class ddeVar:
    ...
    def __init__(self, g, tc=tf.constant([0.], dtype=tf.float32), sign_flag=1): 
        ...
        self.interpolator = scipy.interpolate.interp1d(
            np.array([tc.numpy() - 1, tc.numpy()]),  # X 
    ...
    def update(self, t, Y):
        ...
        self.interpolator = scipy.interpolate.interp1d(
            ...
            np.vstack([self.interpolator.y, Y.flatten()]),  # Y
            kind="linear", axis=0,
            bounds_error=False,
            fill_value=Y.flatten()
        )

    def __call__(self, t=0.):
        """ Y(t) will return the instance's value at time t """

        return self.g(tf.convert_to_tensor(t, tf.float32)) if (t <= self.tc.numpy() and (self.sign_flag == 1 or self.sign_flag == 0)) or (t >= self.tc.numpy() and self.sign_flag == -1) else tf.convert_to_tensor(np.reshape(self.interpolator(t), self.data_shape), tf.float32)
...
class dde(scipy.integrate.ode):
    ...
    def set_initial_value(self, Y):
        ...
        scipy.integrate.ode.set_initial_value(self, Y(Y.tc).numpy().flatten(), Y.tc.numpy())
        
def ddeinttf(func, g, tt, fargs=None):
    ...
    dde_.set_integrator('dopri5', nsteps=5)
#     dde_.set_integrator('vode')
