...
class adj_eqn:
    ...
    def __call__(self, y, t, d, x, batch_size, t_start = np.array([0.])):
        ...
        for k in np.arange(d[0]):
            ...
            input = self.model.process_input(x, t + k*d[1], d)

            dh_dx_i = self.calc_jac(input)
...
class adj_eqn_ODE:
    ...
    def __call__(self, y, t, d, x, batch_size, t_start = np.array([0.])):
        ...
        input = self.model.process_input(x, t, d)
...
class grad_train_var:
    ...
    def calc_jac(self, input):
        with tf.GradientTape() as tape:
            ...
            h = self.model.call_nn_part(input)
    ...
    def lam_dfdp(self, t):
        ...
        input = self.model.process_input(self.y, t, self.d)
