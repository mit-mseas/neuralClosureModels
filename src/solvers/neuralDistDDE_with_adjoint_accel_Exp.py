...
from src.solvers.neuralDDE_with_adjoint_accel_Exp import *
...
class nddde_adj_eqn:
    ...
    @tf.function
    def calc_jac_main(self, z_y_in, channels_to_add):
        with tf.GradientTape() as tape:
            tape.watch(z_y_in)
            f_zy = self.f(z_y_in, channels_to_add)
    ...
    def __call__(self, lam_mu, t, d, z_y, t_start):
        ...
        input, channels_to_add = self.model.process_input(z_y, t, d, t_start)
        
        z_y_in = input[0]
        z = input[0][:, :self.args.state_dim]
        
        df_dzy = self.calc_jac_main(z_y_in, channels_to_add)
