...
class arguments:
    def __init__(self, data_size = 1000, batch_time = 12, batch_time_skip = 2, batch_size = 5, epochs = 500, learning_rate = 0.05, decay_rate = 0.95, test_freq = 1, plot_freq = 2, 
                 ...
                 model_dir = 'DDE_runs/model_dir_test', restart = 0, val_percentage = 0.2, isplot = True):
...
class create_batch:
    ...
    def get_batch(self, flag = 0):
        ...
        if flag == 0:
            ...
            # batch_size = 1
            # batch_time = np.ceil(self.args.data_size/30).astype(int)
            # s = np.asarray([0])
            batch_size = self.args.batch_size
            batch_time = self.args.batch_time
        ...
        return tf.squeeze(batch_y0, axis=-2), batch_t0, batch_t, tf.squeeze(batch_y, axis=-2)
...
class adj_eqn:
    ...
    def __call__(self, y, t, d, x, batch_size):
        ...
        for k in np.arange(d[0]):
            ...
            for i in np.flip(np.arange(d[0])): # d is a list and d[0] contains the number of steps, while d[1] contains the time-step value to skip
                input.append(tf.expand_dims(x(t + k*d[1] - i*d[1]), axis=0))
            ...
            with tf.GradientTape() as tape:
                tape.watch(input)
                h_x = input
                for i in range(len(self.h.layers)):
                    h_x = self.h.layers[i](h_x)

            dh_dx_i = tape.batch_jacobian(h_x, input)
...
class grad_train_var:
    ...
    def __init__(self, model, lam, y, t_lowerlim, t_upperlim, d, args):
        self.model = model
        self.lam = lam
        self.y = y
...
class train_nDDE:
    ...
    def train(self, true_y, true_y0, t, val_true_y):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            if epoch == 1:
#                 pred_y = ddeinttf(self.func, true_y0, t, fargs=(self.d,))
#                 loss_train = tf.squeeze(self.loss(true_y, pred_y))
                
#                 val_pred_y = self.val_obj.data(self.func, true_y, t, self.d)
#                 loss_val = tf.squeeze(self.loss(val_true_y, val_pred_y))

                pred_y = ddeinttf(self.func, true_y0, tf.concat([t, self.val_obj.val_t], axis=0), fargs=(self.d,))
            ...
            for itr in tqdm(range(1, self.args.niters + 1), desc = 'Iterations', file=sys.stdout):
                ...
                if itr == 1:
                    batch_y0, batch_t0, batch_t, batch_y = self.batch_obj.get_batch(1)
                    batch_size = self.args.batch_size
                else:
                    batch_y0, batch_t0, batch_t, batch_y = self.batch_obj.get_batch()
                    batch_size = self.args.batch_size
                ...
                pred_y = ddeinttf(self.func, create_interpolator(batch_y0, batch_t0), batch_t, fargs=(self.d,))
                ...
                for k in range(len(batch_t)-1, 0, -1):
                    ...
                    pred_adj_intrvl = ddeinttf(self.adj_func, adj_eqn_ic_interp, batch_adj_t_intrvl, fargs = (self.d, pred_y0_y_interp, batch_size))
                    ...
                    grads = grad_train_var(self.func, create_interpolator(pred_adj_intrvl, batch_adj_t_intrvl), pred_y0_y_interp, batch_adj_t_intrvl[-1], 
                                        batch_adj_t_intrvl[0], self.d, self.args).grad
                ...
                # print('Epoch: ', epoch, ' | Iteration: ', itr, ' | Learning Rate: ', self.optimizer.learning_rate(self.optimizer.iterations.numpy()).numpy())
...
#                 pred_y = ddeinttf(self.func, true_y0, t, fargs=(self.d,))
#                 loss_train = tf.squeeze(self.loss(true_y, pred_y)) #tf.reduce_mean(tf.keras.losses.MSE(pred_y, true_y)) 
...
class create_validation_set:
    ...
    def data(self, func, y, t, d = None):
        ...
        if d != None:
            val_true_y = ddeinttf(func, self.val_true_y0, self.val_t, fargs=(d,))
        else:
            val_true_y = ddeinttf(func, self.val_true_y0, self.val_t)
        return val_true_y
