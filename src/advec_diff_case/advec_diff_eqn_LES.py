...
class ad_eqn_args:
    def __init__(self, T, nt, L, nx, Re, u_bc_0, u_bc_L, multi_solve_size, Cs):
        self.T = T
        self.nt = nt
...
class operators:
    ...
    def ev_coeff(self, u_left, u_right):
        
        return (self.app.Cs*self.app.dx)**2 * np.abs((u_right - u_left)/(2*self.app.dx))
    ...
    def ev_diff(self, u):
        ...
        for j in range(u.shape[0]):
            ...
            bc_mat_indices.append([j, 0])
            bc_mat_values.append((self.ev_coeff(self.app.u_bc_0, u[j, 1])/self.app.dx**2) * self.app.u_bc_0)
            bc_mat_indices.append([j, self.app.nx-1])
            bc_mat_values.append((self.ev_coeff(u[j, self.app.nx-2], self.app.u_bc_L)/self.app.dx**2) * self.app.u_bc_L)
        
            for i in range(self.app.nx):
                if i == 0:
                    ev = self.ev_coeff(self.app.u_bc_0, u[j, 1])
                elif i == self.app.nx-1:
                    ev = self.ev_coeff(u[j, self.app.nx-2], self.app.u_bc_L)
                else:
                    ev = self.ev_coeff(u[j, i - 1], u[j, i + 1])

                main_mat_indices.append([j, i, i])
                main_mat_values.append((ev/self.app.dx**2) * (-2.))
                if i != 0: 
                    main_mat_indices.append([j, i, i-1])
                    main_mat_values.append((ev/self.app.dx**2) * 1.)
                if i != self.app.nx-1: 
                    main_mat_indices.append([j, i, i+1])
                    main_mat_values.append((ev/self.app.dx**2) * 1.)

        ev_diff_mat = tf.sparse.to_dense(tf.sparse.SparseTensor(main_mat_indices, 
                                                                                                 main_mat_values, dense_shape=[u.shape[0], self.app.nx, self.app.nx]), validate_indices=False)
        ev_diff_bc = tf.sparse.to_dense(tf.sparse.SparseTensor(bc_mat_indices, 
                                                                                                bc_mat_values, dense_shape=[u.shape[0], self.app.nx]), validate_indices=False)
        
        return ev_diff_mat, ev_diff_bc
...
class ad_eqn:
    ...
    def __call__(self, u, t):
        ...
        ev_diff_mat, ev_diff_bc = self.op.ev_diff(u_t)
        ev_diff_term = tf.cast(tf.einsum('abc, ac -> ab', tf.cast(ev_diff_mat, tf.float64), tf.cast(u_t, tf.float64)), tf.float32) + tf.cast(ev_diff_bc, tf.float32)

        dudt = diff_term - adv_term + ev_diff_term
