...
t_modes = tf.linspace(0., 2.0, 200) # Time array

u_fom_modes = ddeinttf(adeq.ad_eqn(op, args), u0, t_modes, alg_name = args.ode_alg_name, nsteps = args.nsteps)

dt_modes = t_modes[1] - t_modes[0]
val_t_len_modes =  1. * (t_modes[-1] - t_modes[0])
n_val_modes = np.ceil(np.abs(val_t_len_modes/dt_modes.numpy())).astype(int)
val_t_modes = tf.linspace(t_modes[-1], t_modes[-1] + val_t_len_modes, n_val_modes)

val_u_fom_modes = ddeinttf(adeq.ad_eqn(op, args), create_interpolator(u_fom_modes, t_modes), val_t_modes, alg_name = args.ode_alg_name, nsteps = args.nsteps)
...
u_mean, ui = create_mean_modes(tf.squeeze(tf.concat([u_fom_modes, val_u_fom_modes], axis=0), axis=1), args, tf.concat([t_modes, val_t_modes], axis=0))()
